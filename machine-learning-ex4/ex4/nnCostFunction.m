...
activation_value = sigmoid(Theta2 * [ones(1, m); sigmoid(Theta1 * [ones(m, 1) X]')]);
tmp_sum = zeros(m, 1);
%size(tmp_y);
%size(activation_value);
for i = 1:m
 tmp_sum(i) = -tmp_y(i,:) * log(activation_value(:, i)) - (1 - tmp_y(i,:)) * log(1 .- activation_value(:,i));
end
